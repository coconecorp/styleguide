...
- 【必須】名前は説明的で、初めて読む人にとっても分かりやすいものでなければなりません。プロジェクトに無関係な読者にとって曖昧で馴染みのない略語を使用したり、単語を省略したりしてはいけません。
...
// 間違い：コメントも付与されていないため、読者にとって何の意味なのかわかりません
...
- 【推奨】ドキュメントには、JSDocのコメント`/** ... */`の使用を推奨します。
- 【推奨】コードの実装に関係するコメントは、通常のコメント`//`の使用を推奨します。

JSDocのコメントは、エディタやドキュメント作成ツールなど、ツールが理解できるコメントです。

通常のコメントは、その他の使用者のためのコメントです。書式のルールはありません。
